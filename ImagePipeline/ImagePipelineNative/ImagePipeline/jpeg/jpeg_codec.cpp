...
extern "C"
{
	#include "libjpeg-turbo\transupp.h"
}
...
namespace facebook 
{
	namespace imagepipeline 
	{
		namespace jpeg 
		{
			...
			static const char* const JPEG_XMP_SEGMENT_HEADER = "http://ns.adobe.com/xap/1.0/";

			/**
			 * The upper bound for xmp metadata length stored in jpeg file.
			 */
			static const unsigned int JPEG_METADATA_LIMIT = 0xFFFF - 2 - JPEG_XMP_SEGMENT_HEADER_LENGTH;
				...
				const DecodedImage& decoded_image) 
			{
				...
				auto jpeg_metadata_writer = [&](int c) 
				{ 
				...
				int quality) 
			{
				...
				JpegOutputStreamWrapper os_wrapper { os };
				...
				int quality) 
			{
				...
				JpegInputStreamWrapper is_wrapper { is };
				JpegOutputStreamWrapper os_wrapper{ os };
				...
				if (should_scale) 
				{
					resizeJpeg(
						...
						should_rotate ? mem_destination.public_fields : os_wrapper.public_fields,
						scale_factor,
						quality);
				}
