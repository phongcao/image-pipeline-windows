...
namespace ImagePipeline.Producers
{
    ...
    ///
    /// <para />The producer uses this interface to notify its client when
    /// new data is ready or an error occurs. Execution of the image request
    /// is structured as a sequence of IProducers. Each one consumes data
    /// produced by producer preceding it in the sequence.
    ///
    /// <para />For example decode is a producer that consumes data produced
    /// by the disk cache get producer.
    ///
    /// <para />The consumer is passed new intermediate results via
    /// OnNewResult(isLast = false) method. Each consumer should expect that
    /// one of the following methods will be called exactly once, as the very
    /// last producer call:
    /// <ul>
    ///   <li>
    ///   OnNewResult(isLast = true) if producer finishes successfully with
    ///   a final result.
    ///   </li>
    ///   <li>
    ...
    ///   </li>
    ///   <li>
    ///   OnCancellation if producer was cancelled before a final result
    ///   could be created.
    ///   </li>
    /// </ul>
    ///
    /// <para />Implementations of this interface must be thread safe, as
    /// callback methods might be called on different threads.
    ///
    /// </summary>
